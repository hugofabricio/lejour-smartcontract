...
class WeddingContract extends Contract {
    ...
    async updateWedding(ctx, registrationId, data) {
        ...
        const findWedding = await ctx.stub.getState(registrationId);
        const weddingData = JSON.parse(findWedding.toString());

        const asset = JSON.parse(data);
        const buffer = Buffer.from(JSON.stringify(Object.assign({}, weddingData, asset)));
    ...
    async readHistory(ctx, registrationId) {
        ...
        const stringWeddingHistory = JSON.stringify(weddingHistory).replace(/\\/g, '');
